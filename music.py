...
class Player(commands.Cog):
    ...
    async def play_song(self, ctx, song):
        ...
        ctx.voice_client.play(discord.PCMVolumeTransformer(discord.FFmpegPCMAudio(url, executable=r"C:\Users\ibj69\Desktop\ghetto groovy\ffmpeg\bin\ffmpeg.exe")), after=lambda error: self.bot.loop.create_task(self.check_queue(ctx)))
        ctx.voice_client.source.volume = 0.5

    @commands.command()
    async def join(self, ctx):
        if ctx.author.voice is None:
            return await ctx.send("You are not connected to a voice channel, please connect ot the channel you want the bot to join.")
    ...
    async def skip(self, ctx):
        ...
        if votes[u"\u2705"] > 0:
            if votes[u"\U0001F6AB"] == 0 or votes[u"\u2705"] / (votes[u"\u2705"] + votes[u"\U0001F6AB"]) > 0.79: # 80% or higher
                skip = True
                embed = discord.Embed(title="Skip Successful", description="***Voting to skip the current song was successful, skipping now.***", colour=discord.Colour.green())
